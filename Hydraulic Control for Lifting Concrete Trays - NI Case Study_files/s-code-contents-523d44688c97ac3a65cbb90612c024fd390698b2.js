...
/* Check if there is DTM Stage script - Start */
var scripts = document.getElementsByTagName( 'script' );
var len = scripts.length
var isThereStageScript = false;
for(var i =0; i < len; i++) {
    if(scripts[i].src.indexOf("satelliteLib") > -1 && scripts[i].src.indexOf("-staging") > -1) {
        isThereStageScript = true;
        break;
    }
}
/* Check if there is DTM Stage script - End */
...
if (typeof NI  === 'undefined') {
	if(location.hostname.indexOf('-test2') > -1) {
		...
		setAccount("-test");
	} else if (location.hostname.indexOf('-dev') === -1 && isThereStageScript == false) {
		setAccount("");
  }
} else if (isThereStageScript == false || NI.env.tier != ""){
 ...
 * Adjusted to get mm-dd:hA format - 8/16/13 - ACE
 */
s._tpDST = {
	2012 : '3/11,11/4',
	2013 : '3/10,11/3',
...
s.getTimeParting = new Function("h", "z", ""
		 ...
		 + ".getMonth()+1;D=d.getDate();U='AM';if(H>=12){U='P"
		 + "M';H=H-12;}if(H==0){H=12;}tm=M+'-'+D+':'+H+U;return(tm);}");
...
if (s.enableVideoTracking) {
	...
	s.Media.trackVars="events,eVar30,prop22,contextData.start,contextData.complete";
